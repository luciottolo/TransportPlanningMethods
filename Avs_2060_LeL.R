...
trips_zones <- trips_zones %>%
  ...
  mutate (trips_prod = trips_prod*1.426,
...
ggplot(zone_112, aes(end_bzname, start_bzname, fill= mean_duration)) + 
  ...
  labs(x = "Destination", y = "Origin",
       title ="Origin-destination matrix - AV 2060",
  ...
  scale_fill_stepsn(breaks = c(0,15,30,55,70,85,100,115,130,145),
                    colours = rev(heat.colors(7,alpha=0.8)),
                    values = scales::rescale(c(0,15,30,55,70,85,100,115,130,145)),
                    show.limits=T)
